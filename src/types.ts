...
// zoho
export const zohoTicketSchema = "zoho_ticket"
...
export const VespaZohoTicketSchema = z.object({
  ...
  firstResponseTime: z.string().optional(), // Timestamp when first response was sent
  resolutionTime: z.string().optional(), // Timestamp when ticket was resolved
  onHoldStartTime: z.string().optional(), // Timestamp when ticket was put on hold
  onHoldEndTime: z.string().optional(), // Timestamp when ticket was taken off hold
  escalatedEndDate: z.string().optional(), // Timestamp when ticket escalation ended
})
...
const ZohoTicketMatchFeaturesSchema = z.object({
  ...
  "closeness(field, subject_embedding)": z.number().optional(),
  "closeness(field, description_embedding)": z.number().optional(),
  "closeness(field, threadSummary_embedding)": z.number().optional(),
  "closeness(field, commentSummary_embedding)": z.number().optional(),
  "closeness(field, wholeResolutionSummary_embedding)": z.number().optional(),
  vector_score: z.number().optional(),
...
export const VespaZohoTicketSearchSchema = VespaZohoTicketSchema.extend({
  sddocname: z.literal(zohoTicketSchema),
...
export const AutocompleteZohoTicketSchema = z
  .object({
    type: z.literal(zohoTicketSchema),
    relevance: z.number(),
    subject: z.string(),
    ticketNumber: z.string(),
    status: z.string(),
...
export interface AppFilter {
  ...
  teamName?: string[]
  accountName?: string[]
  email?: string[] // Contact email who raised the ticket (deprecated: use contactEmail)
  contactEmail?: string[] // Contact email who raised the ticket (preferred)
  cf_merchant_id_1?: string[] // Deprecated: use merchantId instead
  merchantId?: string[] // Merchant identifier (preferred)
  isOverDue?: boolean
...
export type VespaQueryConfig = {
  ...
  departmentIds?: string[] // User's department ID(s) for Zoho permission filtering. Supports multiple departments.
}
...
export interface SearchZohoDeskParams {
  email: string // User's email for logging/audit
  departmentIds?: string[] // User's department ID(s) for permission filtering. Supports multiple departments.
  query?: string
  limit?: number
  offset?: number
  sortBy?: "asc" | "desc"
  ...
  // Note: departmentIds for filtering is now in appFilters
