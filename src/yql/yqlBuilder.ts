...
export class YqlBuilder {
     ...
     */
    nearestNeighbor(field: FieldName, queryParam: string = 'e', targetHits?: number): this {
        const hits = targetHits || this.options.targetHits;
    ...
    hybridSearch(
        ...
        const vectorSearch = new NearestNeighborCondition(vectorField, vectorParam, hits);
    ...
    private validateYqlSyntax(yql: string): void {
        ...
        if (missingKeywords.length > 0) {
            throw new Error(`Missing required YQL keywords: ${missingKeywords.join(', ')}`);
        }
     ...
     */
    static create(userEmail: string, options?: Partial<YqlBuilderOptions>): YqlBuilder {
