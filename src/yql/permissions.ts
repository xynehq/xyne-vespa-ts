...
import { BaseCondition, VespaField, OrCondition, AndCondition } from './conditions';
...
export class PermissionWrapper {
    ...
    wrapEmailPermission(condition: YqlCondition, requirePermissions: boolean = true): YqlCondition {
        if (!requirePermissions) {
            return condition;
        }

        const permissionCondition = PermissionCondition.EmailPermissions(this.userEmail);
        return new AndCondition([condition, permissionCondition]);
    }

    wrapEmailOwner(condition: YqlCondition, requirePermissions: boolean = true): YqlCondition {
        if (!requirePermissions) {
            return condition;
        }
