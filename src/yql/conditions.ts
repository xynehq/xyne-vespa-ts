...
export class AndCondition extends BaseCondition {
    constructor(
        ...
        private permissionType: PermissionFieldType | PermissionFieldType.OWNER = PermissionFieldType.PERMISSIONS
    ) {
        super();
    ...
    toString(): string {
        ...
        if (this.requirePermissions) {
            const permissionField = this.permissionType === PermissionFieldType.OWNER ? PermissionFieldType.OWNER : PermissionFieldType.PERMISSIONS;
    ...
    add(condition: YqlCondition): AndCondition {
        return new AndCondition([...this.conditions, condition], this.requirePermissions, this.userEmail, this.permissionType);
    }

    /**
     * Creates an AndCondition with automatic email permission checking (permissions field) - explicit method
     */
    static withEmailPermissions(conditions: YqlCondition[], userEmail: string = '@email'): AndCondition {
        return new AndCondition(conditions, true, userEmail, PermissionFieldType.PERMISSIONS);
    }

    /**
     * Creates an AndCondition with automatic owner permission checking (owner field)
     */
    static withOwnerPermissions(conditions: YqlCondition[], userEmail: string = '@email'): AndCondition {
        return new AndCondition(conditions, true, userEmail, PermissionFieldType.OWNER);
    }
...
export class OrCondition extends BaseCondition {
    ...
    toString(): string {
        ...
        if (this.requirePermissions) {
            ...
            const permissionField = this.permissionType === PermissionFieldType.OWNER ?
    ...
    add(condition: YqlCondition, requirePermissions: boolean = this.requirePermissions): OrCondition {
        return new OrCondition([...this.conditions, condition], requirePermissions, this.userEmail, this.permissionType);
    }

    /**
     * Creates an OrCondition with automatic email permission checking (permissions field) - explicit method
     */
    static withEmailPermissions(conditions: YqlCondition[], userEmail: string = '@email'): OrCondition {
        return new OrCondition(conditions, true, userEmail, PermissionFieldType.PERMISSIONS);
    }

    /**
     * Creates an OrCondition with automatic owner permission checking (owner field)
     */
    static withOwnerPermissions(conditions: YqlCondition[], userEmail: string = '@email'): OrCondition {
        return new OrCondition(conditions, true, userEmail, PermissionFieldType.OWNER);
    }
...
export class ExclusionCondition extends BaseCondition {
    ...
    isEmpty(): boolean {
        return !this.docIds || this.docIds.length === 0 ||
...
export class InclusionCondition extends BaseCondition {
    ...
    toString(): string {
        ...
        const conditions = this.values
            ...
            .map(value => `${this.field} contains '${escapeYqlValue(value.trim())}'`);
